...
data_simpleBayes <- list(
  N = nrow(s_df),
  rating = s_df$choice2, # the outcome of combining choice1 and group rating
...
```{r}
# setting it up for stan and fitting simple bayes
df_cs <- df_cs %>% 
  filter(ID == 1, GroupRating != 0) %>% # remove conditions without group ratings
  mutate(Source1 = FirstRating/9, # make sources on 0-1 space
         Source2 = GroupRating/9)


data_simpleBayes <- list(
  N = nrow(df_cs),
  rating = df_cs$SecondRating, # outcome of combining both information sources
  Source1 = df_cs$Source1,
  Source2 = df_cs$Source2
)
...
samples_cs <- mod_simpleBayes$sample(
  data = data_simpleBayes,
  #fixed_param = TRUE,
  seed = 123,
  chains = 2,
...
draws_df <- as_draws_df(samples_cs$draws())
...
```{r}
# fitting wegihted bayes to empirical data 
data_weightedBayes <- list(
  N = nrow(df_cs),
  choice = df_cs$SecondRating, # outcome of combining both information sources
  Source1 = df_cs$Source1,
  Source2 = df_cs$Source2
)
